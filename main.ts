...
async function getYesterdayEntries() {
  ...
  return filteredResults.map((page: any) => ({
    id: page.id,
    // Notion 통합이 해석하지 못하는 사람(게스트/외부 유저 등)은 person이 없어 undefined.email 접근 시 크래시가 났음. 옵셔널 체이닝 + falsy 제거.
    user:
      page.properties.사람?.people
        ?.map((user) => user.person?.email)
        .filter(Boolean) || [],
    categories:
      page.properties.분야?.multi_select?.map((tag) => tag.name) || [],
  }));
}
...
async function getRestUsers() {
  ...
  return response.results.map((page: any) => ({
    // 휴식 DB에는 person이 해석되지 않는 항목이 다수 존재(이 줄이 알람 미발송의 근본 원인이었음). 옵셔널 체이닝 + falsy 제거.
    email: page.properties.사람?.people
      ?.map((user) => user.person?.email)
      .filter(Boolean),
  }));
}
    ...
    const writtenUsers = new Set(
      yesterdayEntries
        .filter(
          (entry) =>
            !(
              entry.categories.length === 1 && entry.categories[0] === "쓰는 중"
            ),
        )
